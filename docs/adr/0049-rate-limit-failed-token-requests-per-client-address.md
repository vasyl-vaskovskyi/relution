...
- **Decision:** Option 3.
  - **Key:** `getRemoteAddr()` as the app sees it. IPv4 addresses are used as they are; IPv6 addresses are reduced to their /64 prefix, because one subscriber usually holds a whole /64. An unparsable address falls into one shared `unknown` bucket. `X-Forwarded-For` and `Forwarded` are **not** trusted.
