...
- **Exported log records** (Level 2, [ADR-0051](../adr/0051-export-application-logs-over-otlp.md)) are the same events as the console lines: formatted message, level, logger, exception and trace context. The appender captures no MDC attribute, argument, marker or key-value pair. `LogSafetyIntegrationTest` runs with log export on and applies the same assertions to the records an in-memory exporter receives, and asserts that no MDC key is exported.
