...
| `GF_DASHBOARDS_DEFAULT_HOME_DASHBOARD_PATH` | `/otel-lgtm/grafana/conf/provisioning/dashboards/custom/appstore-service.json` | `compose.observability.yml` (on `otel-lgtm`): Grafana opens on the "App Store service" dashboard ([`observability.md`](observability.md#dashboard)) |
...
- **Log export needs both variables:** `management.logging.export.enabled=true` also attaches the OpenTelemetry Logback appender (ADR-0051), and Boot creates the OTLP log exporter only when `management.opentelemetry.logging.export.otlp.endpoint` is set ([`observability.md`](observability.md#level-2-opentelemetry--grafana-lgtm-stretch-goal)).
