...
## 10 — A fault-injection test that never injected the fault (day)
- **When:** day, Search block, `ItunesSearchClientTest`
- **Suggested:** To cover "the connection breaks after it was established", the assistant used WireMock's `Fault.CONNECTION_RESET_BY_PEER` and asserted `UpstreamServerErrorException` with status 0.
- **Problem:** The test failed with status **500**. With WireMock 3.13 on Jetty 12, that fault comes back as an ordinary HTTP 500, so the test never exercised the path it was named after. The first replacement, a raw socket that accepted one connection and closed it, then ended in a **read timeout**: the JDK `HttpClient` silently retries an idempotent GET once on a new connection when the first one closes before any response, and nothing accepted the second connection.
- **Outcome:** Corrected. The raw socket now accepts and closes every connection. The test asserts both the classification and that the client opened exactly 2 connections. The hidden JDK retry is documented in [`../architecture/caching-resilience.md`](../architecture/caching-resilience.md#retry), because it affects the retry and latency reasoning.
- **Lesson:** A fault-injection test must assert what the fake server actually saw, not only what the client reported. The HTTP client's built-in behavior is part of the retry design.

## 11 — "Caffeine removes failed futures" was true, but not soon enough (day)
- **When:** day, caching and resilience block, `AppSearchService` / `AppDetailsService`
- **Suggested:** The design (ADR-0030, written with AI help in prep) said "failures are never cached: Caffeine removes a future that completes exceptionally". The assistant implemented the services on that basis: `cache.get(...)`, then `join()`, then rethrow the cause.
- **Problem:** A test for exactly that claim failed. A request right after a failed load received the same failure again. Caffeine does remove the failed future, but in a completion callback on the loader thread, after the waiting caller has already been woken, so an immediate retry can still find the failed entry. A second test failed for the same reason: the fake ticker moved before Caffeine recorded the entry's write time, so a TTL check looked wrong.
- **Outcome:** Corrected in production code. `CacheSupport.getOrLoad` removes exactly the failed future (`asMap().remove(key, future)`) before rethrowing, and a test repeats failure-then-success 25 times on the real virtual-thread executor. TTL tests now load on the calling thread. The docs describe the asynchronous removal.
- **Lesson:** "The library handles it" needs a test that runs on the real concurrency setup. Ask when a guarantee takes effect, not only whether it exists.
