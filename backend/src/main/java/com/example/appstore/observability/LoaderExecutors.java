 ...
 * own virtual threads, and the MDC keys below are copied from the submitting thread
...
public final class LoaderExecutors {
    ...
    public static final List<String> PROPAGATED_MDC_KEYS = List.of("correlationId", "clientId");

    private LoaderExecutors() {}

    /** A virtual-thread-per-task executor that propagates {@link #PROPAGATED_MDC_KEYS}. The caller must close it. */
