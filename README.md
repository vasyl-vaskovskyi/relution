...
**Optional observability stack:** run `cp .env.observability.example .env.observability`, set `APPSTORE_GRAFANA_ADMIN_PASSWORD` there (not in `.env`, which the app container receives), then run `docker compose --env-file .env.observability -f docker-compose.yml -f compose.observability.yml up --build`. It exports traces, metrics and logs to Grafana at http://localhost:3000, which opens on the provisioned "App Store service" dashboard; `scripts/demo-traffic.sh` sends a small request mix so every panel has data (demo use only; see [`docs/operations/observability.md`](docs/operations/observability.md#level-2-opentelemetry--grafana-lgtm-stretch-goal)).
